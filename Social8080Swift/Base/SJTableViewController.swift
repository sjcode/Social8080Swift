...
class SJTableViewController: SJViewController {
    ...
    lazy var tableView : UITableView = {
        let v = UITableView(frame: CGRect(x: 0, y: 0, width: ScreenSize.SCREEN_WIDTH, height: ScreenSize.SCREEN_HEIGHT - ControlSize.TABBAR_HEIGHT), style: .Grouped)
        v.mj_header = MJRefreshNormalHeader(refreshingTarget: self, refreshingAction: #selector(loadData))
        v.delegate = self
        v.dataSource = self
        let refresh = v.mj_header
        let f = refresh.frame
        //let newframe = CGRectMake(f.origin.x, f.origin.y - 20, f.size.width, f.size.height)
